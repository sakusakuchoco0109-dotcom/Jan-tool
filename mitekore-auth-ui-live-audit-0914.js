...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(({ bad }) => {
      ...
      if (!t || t.includes('確認コードを確認しています')) return false;
      return { text: t, bad: t.includes(bad) };
    ...
    fs.writeFileSync('mitekore-auth-ui-live-audit-0914-result.json', JSON.stringify(out, null, 2));
